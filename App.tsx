...
const App: React.FC = () => {
  ...
  // We use a fixed reference for rotation compensation to prevent jitter
   ...
   * @param forceTransparent If true, ignores the background selection and exports transparent PNG (Layer mode)
   */
  const exportAsset = async (forceTransparent: boolean = false) => {
    ...
    // Standard HD Broadcast resolution
    canvas.width = 1920;
    canvas.height = 1080;
    ...
    const drawHeading = (num: number) => {
      ...
      // Logic must match CSS transforms exactly
      const angleVal = num === 1 ? BRAND.metrics.angle : -BRAND.metrics.angle;
      const rot = angleVal * (Math.PI / 180);
      const text = (num === 1 ? text1 : text2) || ' ';
      
      // Calculate Vertical Positioning based on group height
      // The group total height is effectively 2 boxes + the offset between them
      // We center that group in the viewport
      const offset = boxStackOffset; 
      
      // Box 1 center Y relative to group center
      const box1Y = -(totalBoxHeight + offset) / 2 + (totalBoxHeight / 2);
      // Box 2 center Y relative to group center
      const box2Y = (totalBoxHeight + offset) / 2 - (totalBoxHeight / 2);
      
      // Actual Y position for this specific box
      ...
      // Translate to center of canvas + shift
      ...
      // Calculate Widths with Tracking
      const tracking = -0.04 * fontSize; // -0.04em tracking
      const chars = [...text];
      const charWidths = chars.map(c => ctx.measureText(c).width);
      const totalTextWidth = charWidths.reduce((a, b) => a + b + tracking, 0) - tracking;
      const boxWidth = totalTextWidth + (brandPadding * 2);

      // Draw Box (Centered at 0,0 locally)
      ctx.fillStyle = colors.bg;
      // Add Shadow only if not a raw layer export (optional, but looks better with shadow usually)
      if (!forceTransparent || true) {
      ...
      // Draw character by character for custom tracking
      let cursorX = -boxWidth/2 + brandPadding;
      // Optical adjustment for Poppins vertical centering
    ...
    // Draw based on stacking order
    if (stacking === 'box1') {
      drawHeading(2); // Bottom first
      drawHeading(1); // Top second
    } else {
      drawHeading(1);
      drawHeading(2);
    }
  ...
  // Helper for transparency checkerboard
  const checkerboardStyle = {
    ...
    backgroundColor: '#1a1f2e' // Darker base for checkerboard
  };
          ...
          <div className="w-full h-full relative flex items-center justify-center">
            ...
            <div 
              ...
              style={{ 
                ...
                // We handle the gap via margin in the second element usually, but flex gap is cleaner for preview if consistent
                // Actually, stick to margin approach to match Canvas logic perfectly
                gap: 0
              }}
            >
